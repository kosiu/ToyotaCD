...
// Analog comparator
    ADCSRB &= ~_BV(ACME);   // Analog Comparator Multiplexer Enable - NO
    ACSR &= ~( _BV(ACIS1) | _BV(ACIS0) );// Analog Comparator Interrupt Mode Select
...
// is this command for me ?
    ...
    if (for_me) {
        ...
        if (CheckCmd((uint8_t*)btn_scan))  {
            answerReq = cmNull;    //button
            return 1;
        }
